...
#define PID 0x0483 //change this to the process ID specific to your device returned by "lsusb"
#define VID 0x16C0 //change this to the vendor ID specific to your device returned by "lsusb"
#define PIPE //change to pipe used by your serial device
void usage(void)
{
...
#if defined(USE_LIBUSB)
// TODO: move macros for teensy ID and ports locally instead of global
...
{
	teensy_close();
	libusb_teensy_handle = open_usb_device(VID/*0x16C0*/, PID/*0x0478*/);
...
int teensy_write(void *buf, int len, double timeout)
{
	...
	if (!libusb_teensy_handle){
                printf("libusb_teensy_handle was lost on write\n");
                return 0;
        }        
	while (timeout > 0) {
		r = usb_control_msg(libusb_teensy_handle, 0x21, 9, 0x0200, 0,
			(char *)buf, len, (int)(timeout * 1000000.0));
		if (r >= 0){
                        printf("%d\n", r);
                        return 1;
                }        
...
int hard_reboot(void)
{
	...
	rebootor = open_usb_device(VID/*0x16C0*/, PID/*0x0477*/);
